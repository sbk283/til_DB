...
- `데이터 중볷이 안되도록 하는 과정` 을 정규화라고 합니다.
- 제 1 정규형, 제 2 정구형, 제 3 정규형 등이 제시됨
...
```
아이디, 패스워드, 이름, 이메일 ,영화이름, 영화출현배우, 영화상영시간
게시글, 게시글제목, 게시글내용, 게시글 생성 시간, 게시글 수정 시간
```

- 위의 컬럼 내용들은 상위 개념으로 묶어본다.

```
아이디, 패스워드, 이름, 이메일 ====> 사용자들
영화이름, 영화출현배우, 영화상영시간 ====> 영화들
게시글, 게시글제목, 게시글내용, 게시글 생성 시간, 게시글 수정 시간 ====> 게시글들
```

## 3. 실제로 고민해서 `Entity` 로 지정해 보기
...
- 저장해야 하는 데이터 들을 분류해서 묶어준다.
- 하나의 그룹이 생성되고 이를 분류한 이름이 `Entity`
- `Entity` 는 `Table` 이면서 `Table 이름` 이다.
- 실습) [실습 스프레드시트](https://docs.google.com/spreadsheets/d/1bvY4KHtHCR1GFPV5lLfytkhchKHNNc99E0PimAen7y0/edit?gid=0#gid=0)
