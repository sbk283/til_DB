...
```sql
insert into posts (id, title, contents, authors_id) values (2, "제목2", "내용2", 3);
select * from posts;
```
...
```sql
select * from authors where name = '또치';
```
...
select name, email from authors; -- 여러 항목
select name from authors where name = '아이유';
```
...
select * from authors order by id asc; -- 오름차순
select * from authors order by id desc; -- 내림차순

-- 이름 정렬
select * from authors order by name;
select * from authors order by name asc; -- 오름차순
select * from authors order by name desc; -- 내림차순
```
...
select * from authors where password = '1234' order by id desc limit 2;
```
