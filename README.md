...
- 현장에서 대화가 되어야 합니다.
- 특히, BE와 FE 가 협업을 하는데 의사소통이 안되면 곤란함.
...
- 회원 테이블 만들기
- 제품 테이블 만들기
- [예제 - 구글 시트](https://docs.google.com/spreadsheets/d/1bvY4KHtHCR1GFPV5lLfytkhchKHNNc99E0PimAen7y0/edit?usp=sharing)

### 4.1. 기본키(Primary Key)

- 흔히 `pk` 라고 하면 의사소통됨
- 수많은 데이터를 구별해 주기 위해서 기본키가 필요함
...
- 데이터가 오류가 없음을 백엔드 분들은 `참조 무결성` 이라고 합니다.
...
- 운영체제와는 상관없도록 반드시 `소문자`를 규칙으로 하면 좋다.

### 5.2. snake_case 를 사용한다.

- 테이블명, 칼럼명을 여러 단어로 작성할때 `소문자_소문자`로 작성한다.
...
### 5.5. 회사명의 이름을 지울 때는 가능하면 복수형을 선택하자.
